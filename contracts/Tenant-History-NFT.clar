...
(define-read-only (is-landlord-authorized (landlord principal) (tenant principal))
  (default-to false (map-get? landlord-permissions { landlord: landlord, tenant: tenant }))
)
...
(define-public (mint-tenant-record
  ...
  (let
    ...
    (asserts! (or (is-eq tx-sender contract-owner) 
                  (is-landlord-authorized tx-sender tenant)) err-not-authorized)
    (asserts! (and (>= payment-score u1) (<= payment-score u10)) err-invalid-rating)
    (asserts! (and (>= condition-score u1) (<= condition-score u10)) err-invalid-rating)
    (asserts! (and (>= overall-rating u1) (<= overall-rating u10)) err-invalid-rating)
    ...
    (update-tenant-stats tenant rent-amount overall-rating (is-eq tx-sender contract-owner))
...
(define-public (grant-landlord-permission (landlord principal))
  (begin
    (map-set landlord-permissions { landlord: landlord, tenant: tx-sender } true)
    (ok true)
  )
)
...
(define-public (revoke-landlord-permission (landlord principal))
  (begin
    (map-delete landlord-permissions { landlord: landlord, tenant: tx-sender })
    (ok true)
  )
)
  ...
  (let
    ...
    (let
      (
        ...
        (stats (default-to 
          { total-records: u0, average-rating: u0, total-rent-paid: u0, verified-records: u0 }
          (map-get? tenant-stats tenant)
        ))
      )
...
(define-public (update-listing-availability (listing-id uint) (available bool))
  (let
    (
      (listing (unwrap! (map-get? property-listings listing-id) err-listing-not-found))
    )
...
(define-private (update-tenant-stats (tenant principal) (rent uint) (rating uint) (verified bool))
  (let
    (
      ...
      (new-verified (if verified (+ (get verified-records current-stats) u1) (get verified-records current-stats)))
      (current-avg (get average-rating current-stats))
      (new-avg (if (is-eq (get total-records current-stats) u0)
                   rating
                   (/ (+ (* current-avg (get total-records current-stats)) rating) new-total-records)))
    )
    (map-set tenant-stats tenant
      {
  ...
  (let
    (
      ...
      (verified-ratio (if (> (get total-records stats) u0)
                          (/ (* (get verified-records stats) u100) (get total-records stats))
                          u0))
      (rating-score (* (get average-rating stats) u10))
      (volume-bonus (if (> (get total-records stats) u5) u50 u0))
    )
